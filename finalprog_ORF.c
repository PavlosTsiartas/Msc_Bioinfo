...
int main()
{
	...
	while (scanf("%s", seq)==1)
	{
		...
		for (i = 0; i < 30; i++) {stopPositions[i] = 0;}//making the array "empty"
		...
		{ 
			int smallP = 30;//start from an utopical value
			for(x=0; x<30; x++)
			{
			...
			{	
				printf("\nPositions of TAA:");
				for(x=0; x<=9; x++) {if(stopPositions[x] !=0){printf("%d, ",stopPositions[x]);}}
			...
			{
				printf("\nPositions of TAG:");
				for(x=10; x<=19; x++) {if(stopPositions[x] !=0){printf("%d, ",stopPositions[x]);}}
			}	
			
			if(STOP3>=1)
			{
				printf("\nPositions of TAG:");
				for(x=20; x<=29; x++) {if(stopPositions[x] !=0){printf("%d, ",stopPositions[x]);}}
			}
